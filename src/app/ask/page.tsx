...
export default function AskPage() {
  ...
  return (
    <ResponsiveLayout headerTitle="Ask" showHeader={true}>
      <div className="ask-container flex flex-col h-[calc(100vh-8rem)] md:h-[calc(100vh-2rem)]">
        {/* PC Header */}
        <h1 className="hidden md:block text-2xl font-bold p-4">Ask AI</h1>
        ...
        <div className="p-4 border-t border-[var(--border)] bg-[var(--background)]">
          <form onSubmit={handleSubmit} className="flex gap-2">
            <input
              ...
              placeholder="何でも聞く..."
              className="flex-1 px-4 py-2 bg-[var(--secondary)] rounded-lg focus:outline-none focus:ring-2 focus:ring-[var(--primary)] md:py-3"
              disabled={ask.isPending}
            />
            <button
              type="submit"
              disabled={!query.trim() || ask.isPending}
              className="px-4 py-2 bg-[var(--primary)] text-white rounded-lg disabled:opacity-50 disabled:cursor-not-allowed hover:opacity-90 transition-opacity md:px-6"
            >
